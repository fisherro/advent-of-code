...
(define (parse-literal pkt bitstream)
  (let loop ((bs bitstream)
             (digits ""))
    (define prefix (substring bs 0 1))
    (define new-digits (string-append digits (substring bs 1 5)))
    (define remaining (substring bs 5))
    (cond ((string=? prefix "1")
           (loop remaining new-digits))
          (else
           (values (literal-packet (packet-version pkt)
                                   (packet-type pkt)
                                   (binary->number new-digits))
                   remaining)))))
